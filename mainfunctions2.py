...
def show_mp(material_properties, top=0, show=0):
    ...
    for i in range(0,top):
        ...
        plt.imshow(color_image)
        if show:
            plt.title(str(material_properties[i][0])+ ',\n rgb: '+ str(material_properties[i][3]))
        #dont show axis
        plt.xticks([])
        plt.yticks([])
        #make titles small
...
def extract_color_value(Specular,material_properties, recurson,hue_material_number, maskv,masked_img, rgb_img , material_colors_img , hue,saturation, show_graphs=False):
    ...
    if hue_material_number !='N':
        ...
        material_properties.append([total_pixels_maskedv,maskv,[material_color_rgb[0][0][0], material_color_rgb[0][0][1], material_color_rgb[0][0][2]], specular_data])
...
def extract_color_new(Specular,material_properties, recurson,hue_material_number, maskec,masked_rgb_img, rgb_img , material_colors_img , hue, show_graphs=False):
    ...
    if (len(s_start_end)<=2):
        ...
        if hue_material_number !='N':
            ...
            material_color_rgb = cv.cvtColor(material_color_hsv, cv.COLOR_HSV2RGB)
            # mat_number, where it acts, rgb alpha, enabled?
            material_properties.append([total_pixels_maskedec,maskec,[material_color_rgb[0][0][0], material_color_rgb[0][0][1], material_color_rgb[0][0][2]], specular_data])
    ...
    if (len(s_start_end)>2):
        ...
        for i in range(0, len(s_start_end)//2):
            ...
            if show_graphs:
                ...
                extract_color_value(Specular, material_properties,1,0 ,maskecnew ,new_image, rgb_img , material_colors_img ,hue,saturation,1)
            if show_graphs==0:
                extract_color_value(Specular, material_properties,1,0 ,maskecnew ,new_image, rgb_img , material_colors_img ,hue,saturation,0)
